...
def plots_dashboard():
    ...
    if 'pareto_front' in st.session_state and 'multiobjective_solutions' in st.session_state:
        ...
        # Reverse the pareto_front for display (lowest CO2 to highest)
        reversed_indices = list(reversed(range(len(st.session_state.pareto_front))))
        co2_vals, npc_vals = zip(*st.session_state.pareto_front)

        # Show dropdown using reversed order
        selected_reversed_index = st.selectbox(
            "Select a solution to visualize",
            reversed_indices,
            index=st.session_state.get('selected_solution_index', 0),
            format_func=lambda i: f"Solution {len(st.session_state.pareto_front) - i}: CO₂ = {co2_vals[i]/1000:.2f} t, NPC = {npc_vals[i]/1000:.2f} k{currency}",
            key="selected_solution_index"
        )

        # Use the selected solution
        model.solution = st.session_state.multiobjective_solutions[selected_reversed_index]
