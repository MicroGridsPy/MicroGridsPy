...
def export_results(project_name: str, model: Model, fig: dict) -> None:
    """Setup the export results section."""
    # Retrieve esults folder path
    results_folder = PathManager.RESULTS_FOLDER_PATH
    plots_folder = PathManager.RESULTS_FOLDER_PATH / "Plots"
    plots_folder.mkdir(exist_ok=True)
    # Retrieve the related project folder path
    path_manager = PathManager(project_name)
    project_folder_path = os.path.join(path_manager.PROJECTS_FOLDER_PATH / project_name / "results")
    project_folder_plots_path = os.path.join(project_folder_path, "Plots")
...
def plots_dashboard():
    ...
    cost_breakdown_fig = costs_pie_chart(model, optimization_goal)
