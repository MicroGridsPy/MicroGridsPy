...
def plots_dashboard():
    ...
    if model.has_generator:
        st.write(f"Multi-Objecive activated: {model.get_settings('multiobjective_optimization', advanced=True)}")
        fuel_emission = model.get_solution_variable('Fuel Emissions').values.item()
        st.metric("Fuel Total Emissions", f"{fuel_emission:.2f} %")
        if partial_load: 
