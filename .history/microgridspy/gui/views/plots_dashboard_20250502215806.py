...
def plots_dashboard():
    ...
    if 'pareto_front' in st.session_state and 'multiobjective_solutions' in st.session_state:
        ...
        # Show dropdown to select a solution
        selected_index = st.selectbox(
            ...
            format_func=lambda i: f"Solution {i + 1}: CO₂ = {co2_vals[i]/1000:.2f} t, NPC = {npc_vals[i]/1000:.2f} k{currency}",
            key="selected_solution_index"
        )

        # Use the selected solution to update the model
