...
def plots_dashboard():
    ...
    partial_load = st.session_state.get('partial_load', False)
    ...
    if 'multiobjective_solutions' in st.session_state:
        ...
        try:
            total_emission = model.get_solution_variable("total_emission").item()
            st.metric("Total CO₂ Emissions", f"{total_emission:.2f} kgCO₂")
        except:
            st.warning("Total emissions variable not found in model.")
        ...
        add_emission_row("Renewables Installation (LCA)", "res_emission")
        if model.has_battery:
            add_emission_row("Battery Installation (LCA)", "battery_emission")
        if model.has_generator:
            add_emission_row("Generator Installation (LCA)", "gen_emission")
            add_emission_row("Generator Fuel Combustion", "fuel_emission")
        if model.has_grid_connection:
            add_emission_row("Grid Import", "grid_emission")
