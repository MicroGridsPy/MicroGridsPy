...
# Function to get the UTC offset
def get_utc_offset(lat: float, lon: float) -> int:
    ...
    utc_offset = timezone.utcoffset(datetime.now(datetime.timezone.utc)).total_seconds() / 3600
...
def handle_location_input(address: str) -> None:
    ...
    try:
        ...
        # Get the UTC offset and store it in session state
        st.session_state.time_zone = get_utc_offset(lat, lon)
        st.success(f"Location found! UTC Offset: {st.session_state.time_zone} hours")
...
def resource_assessment():
    ...
    if address: handle_location_input(address)
    ...
    if output and output.get('last_clicked'):
        ...
        # Get the UTC offset and store it in session state
        st.session_state.time_zone = get_utc_offset(st.session_state.lat, st.session_state.lon)

    st.write(f"Selected Coordinates: {st.session_state.lat}, {st.session_state.lon}")
    st.write(f"UTC Offset: {st.session_state.time_zone} hours")
