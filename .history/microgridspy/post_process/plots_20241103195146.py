...
def create_sizing_plot(model: Model, color_dict: dict, sizing_df: pd.DataFrame):
    """
    Create a simplified plot to visualize existing and final capacities,
    and a waterfall plot for capacity expansion if applicable.
    """
    ...
    # Determine if there is capacity expansion
    num_steps = capacities.shape[1]  # Number of investment steps
    is_expansion = num_steps > 1
    ...
    # Create a simple bar plot for existing and final capacities
    fig, ax = plt.subplots(figsize=(10, 5))
    final_capacities = capacities[:, -1]  # Final capacities after all investment steps

    # Plot existing capacities in one shade and final capacities in a different shade
    ax.bar(categories, existing_capacities, color=[c + '80' for c in colors], label='Existing Capacity')  # Lighter shade
    ax.bar(categories, final_capacities, color=colors, label='Final Capacity', alpha=0.8)  # Original color
    ...
    # If there are multiple investment steps, add a waterfall plot
    if is_expansion:
        fig, ax = plt.subplots(figsize=(10, 5))
        for index, category in enumerate(categories):
            expansions = [capacities[index, step] - (capacities[index, step - 1] if step > 0 else existing_capacities[index])
                          for step in range(num_steps)]
            ax.bar(range(num_steps), expansions, color=colors[index], label=category if index == 0 else "")

        ax.set_ylabel('Marginal Increase in Capacity')
        ax.set_xlabel('Investment Step')
        ax.set_title('Capacity Expansion Waterfall Plot')
        plt.xticks(range(num_steps), [f'Step {i + 1}' for i in range(num_steps)])
        plt.tight_layout()
