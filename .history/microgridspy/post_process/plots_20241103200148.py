...
def create_sizing_plot(model: Model, color_dict: dict, sizing_df: pd.DataFrame):
    ...
    ax.bar(categories, existing_capacities, color=colors, alpha=1.0, label='Existing Capacity')  # Full opacity

    # Plot the additional capacities using the same colors but with reduced alpha
    ax.bar(categories, total_capacities - existing_capacities, color=colors, alpha=0.5, bottom=existing_capacities, label='New Capacity')  # Reduced opacity
