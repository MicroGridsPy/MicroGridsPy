...
def create_sizing_plot(model: Model, color_dict: dict, sizing_df: pd.DataFrame):
    """
    Create a simplified bar plot to visualize existing and total capacities.
    The existing capacity section will be displayed with a darker shade.
    """
    # Extract data from sizing_df
    categories = sizing_df['Component'].tolist()
    existing_capacities = sizing_df['Existing'].astype(int).values
    total_capacities = sizing_df['Total'].astype(int).values  # Extract total capacities
    ...
    # Plot existing capacities with a darker shade
    ax.bar(categories, existing_capacities, color=colors, alpha=0.8, label='Existing Capacity')

    # Plot the total capacities, stacking on top of the existing capacities
    ax.bar(categories, total_capacities - existing_capacities, color=[c + '80' for c in colors], bottom=existing_capacities, label='New Capacity')
