...
def get_sizing_results(model: Model) -> pd.DataFrame:
    """
    Get and format the sizing results, including existing capacities for brownfield scenarios.
    
    Args:
        model (Model): The optimization model object.
        
    Returns:
        pd.DataFrame: A DataFrame with the formatted sizing results.
    """
    categories = []
    capacities = []
    existing_capacities = []
    ...
    for name, units, nominal_capacity, existing_capacity in components_kw:
        ...
        existing_capacities.append(np.round(existing_capacity))  # Rounded
    ...
    if model.has_battery:
        ...
        if bat_units is not None:
            ...
            existing_capacities.append(np.round(battery_existing_capacity))  # Rounded
    ...
    columns = ['Component', 'Existing'] + [f'Step {i+1}' for i in range(len(capacities[0]))]
