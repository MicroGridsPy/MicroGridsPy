...
def add_grid_constraints(model: Model, settings: ProjectParameters, sets: xr.Dataset, param: xr.Dataset, var: Dict[str, linopy.Variable]) -> None:
    ...
    add_maximum_energy_supply_constraint(model, settings, sets, param, var)
...
def add_maximum_energy_supply_constraint(model: Model, settings: ProjectParameters, sets: xr.Dataset, param: xr.Dataset, var: Dict[str, linopy.Variable]) -> None:
    """Add constraint for maximum energy supply from the grid."""
    ...
    for year in sets.years.values:
        if year >= year_grid_connection:
            ...
            model.add_constraints(
                ...
                name=f"No Energy Supply From Grid - Year {year}")
