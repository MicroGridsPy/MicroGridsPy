...
def add_generator_emissions_constraints(model: Model, settings: ProjectParameters, sets: xr.Dataset, param: xr.Dataset, var: Dict[str, linopy.Variable]) -> None:
    ...
    # Fuel emissions
    fuel_emission_expr = var['generator_fuel_consumption'] * param['FUEL_CO2_EMISSION']
    st.warning("[DEBUG] Generator Fuel Consumption:", var['generator_fuel_consumption'])
    st.warning("[DEBUG] Fuel CO2 Emission Factor:", param['FUEL_CO2_EMISSION'])
    st.warning("[DEBUG] Fuel Emission Expression:", fuel_emission_expr)

    model.add_constraints(var['fuel_emission'] == fuel_emission_expr, name="Fuel Emissions Constraint")
