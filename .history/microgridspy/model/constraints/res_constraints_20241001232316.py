...
def add_renewable_energy_production_constraints(model: Model, settings: ProjectParameters, sets: xr.Dataset, param: xr.Dataset, var: Dict[str, linopy.Variable]) -> None:
    ...
    print("DEBUG")
    print(var['res_units'].sel(steps=step).shape)
    print(param['RES_EXISTING_CAPACITY'].shape)
    print(param['RES_NOMINAL_CAPACITY'].shape)
    print(param['RESOURCE'].shape)
    print(param['RES_INVERTER_EFFICIENCY'].shape)
