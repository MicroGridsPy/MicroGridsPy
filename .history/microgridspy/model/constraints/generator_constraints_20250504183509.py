...
def add_generator_fuel_consumption_constraints(model: Model, settings: ProjectParameters, sets: xr.Dataset, param: xr.Dataset, var: Dict[str, linopy.Variable]) -> None:
    ...
    for year in years:
        ...
        for gen in sets.generator_types.values:
            ...
            # Add fuel consumption limits if applicable
            if param['MAXIMUM_FUEL_CONSUMPTION'].sel(generator_types=gen) > 0:
                yearly_fuel_consumption = (var['generator_fuel_consumption'].sum('periods')).sel(years=year, generator_types=gen)
                model.add_constraints(yearly_fuel_consumption <= param['MAXIMUM_FUEL_CONSUMPTION'].sel(generator_types=gen), name=f"Fuel Consumption Limit Constraint - Year {year}, Type {gen}")
