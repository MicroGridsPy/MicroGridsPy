...
def add_generator_emissions_constraints(model: Model, settings: ProjectParameters, sets: xr.Dataset, param: xr.Dataset, var: Dict[str, linopy.Variable]) -> None:
    ...
    if settings.generator_params.partial_load:
        model.add_constraints(var['fuel_emission'] == var['generator_fuel_consumption'] * param['GENERATOR_FUEL_LHV'] * param['GENERATOR_FUEL_CO2_EMISSION'], name="Fuel Emissions Constraint")
...
def add_project_emissions(
    ...
    if has_generator:
        ...
        total_emissions += var['fuel_emission']
    ...
    try:
        ...
        model.add_constraints(var['total_emission'] == (var["scenario_co2_emission"] * param['SCENARIO_WEIGHTS']).sum('scenarios'))
