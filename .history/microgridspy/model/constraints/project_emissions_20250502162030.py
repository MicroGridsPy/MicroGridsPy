...
def add_generator_emissions_constraints(model: Model, settings: ProjectParameters, sets: xr.Dataset, param: xr.Dataset, var: Dict[str, linopy.Variable]) -> None:
    ...
    st.write("[DEBUG] Generator Fuel Consumption:", var['generator_fuel_consumption'])
    st.write("[DEBUG] Fuel CO2 Emission Factor:", param['FUEL_CO2_EMISSION'])
    st.write("[DEBUG] Fuel Emission Expression:", fuel_emission_expr)
