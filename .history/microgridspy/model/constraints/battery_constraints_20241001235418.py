...
def add_battery_state_of_charge_constraints(model: Model, settings: ProjectParameters, sets: xr.Dataset, param: xr.Dataset, var: Dict[str, linopy.Variable]) -> None:
    ...
    # Contribution from existing capacity (isolated)
    if is_brownfield:
        first_year_first_period_existing = (
            param['BATTERY_EXISTING_CAPACITY'] * param['BATTERY_INITIAL_SOC']
        ).where(is_first_year & is_first_period, 0)
    else:
        first_year_first_period_existing = 0

    # Combine both components for the first year and first period
    first_year_first_period = first_year_first_period_units + first_year_first_period_existing
...
def add_battery_emissions_constraints(model: Model, settings: ProjectParameters, sets: xr.Dataset, param: xr.Dataset, var: Dict[str, linopy.Variable]) -> None:
    ...
    # Add the constraint
    model.add_constraints(var['battery_emissions'] == battery_emissions, name="Battery Emissions Constraint")
