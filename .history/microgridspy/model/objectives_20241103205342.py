...
def add_npc_objective(model: Model, settings: ProjectParameters, sets: xr.Dataset, param: xr.Dataset, var: Dict[str, linopy.Variable]) -> None:
    """
    Define and add NPC objective functions to the Linopy model based on project parameters.
...
def add_emissions_objective(model: Model, settings: ProjectParameters, sets: xr.Dataset, param: xr.Dataset, var: Dict[str, linopy.Variable]) -> None:
    """
    Define and add Total Emissions objective function to the Linopy model.
    ...
    """
    # Net Present Cost Objective
    objective = (var["scenario_co2_emission"] * param['SCENARIO_WEIGHTS']).sum('scenarios')
    model.add_objective(objective)
