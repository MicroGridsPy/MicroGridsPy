...
from microgridspy.model.objectives import add_npc_objective, add_emissions_objective
...
class Model:
    ...
    def solve_multi_objective(self, num_points: int) -> List[Tuple[float, float]]:
        ...
        add_npc_objective(self.model, self.settings, self.sets, self.parameters, self.variables)
        self.model.solve()
        max_co2 = self.model.solution.objective_value

        # Step 2: Minimize CO₂ emissions without NPC constraint (max NPC)
        self.model.clear_objectives()
        add_emissions_objective(self.model, self.settings, self.sets, self.parameters, self.variables)
        self.model.solve()
