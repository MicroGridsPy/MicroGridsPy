...
def select_dispatch_view(
    dispatch: pd.DataFrame, data: xr.Dataset, *, mode: str, scenario_label: str | None
) -> pd.DataFrame:
    frame = dispatch.copy()
    ...
    if mode == "scenario" and scenario_label is not None:
        return (
            frame[frame["scenario"] == str(scenario_label)]
            .sort_values("period")
    ...
    expected = (
        pd.concat([frame[["period"]], weighted], axis=1).groupby("period", as_index=False).sum()
    )
...
def select_kpi_row(
    kpis: pd.DataFrame, data: xr.Dataset, *, mode: str, scenario_label: str | None
) -> pd.Series:
    frame = kpis.copy()
    frame["scenario"] = frame["scenario"].astype(str)
    if mode == "scenario" and scenario_label is not None:
    ...
    for col in numeric_cols:
        if col in {"objective_value", "solver_objective_value"}:
            values[col] = (
                float(pd.to_numeric(scenario_rows[col], errors="coerce").fillna(0.0).iloc[0])
            ...
            values[col] = float(
                (
                    pd.to_numeric(scenario_rows[col], errors="coerce").fillna(0.0)
                    * scenario_rows["weight_fallback"]
                ).sum()
            )
...
def build_reporting_tables(
    *,
    ...
    if "period" in dispatch.columns:
        dispatch["period"] = (
            pd.to_numeric(dispatch["period"], errors="coerce").fillna(0).astype(int)
        )
    ...
    dispatch["grid_import_delivered"] = pd.to_numeric(
        dispatch["grid_import_delivered"], errors="coerce"
    ).fillna(np.nan)
    dispatch["grid_export_delivered"] = pd.to_numeric(
        dispatch["grid_export_delivered"], errors="coerce"
    ).fillna(np.nan)
    for s in scenarios:
        mask = dispatch["scenario"] == s
        grid_eta = (
            scalarize(data["grid_transmission_efficiency"], scenario=s)
            if "grid_transmission_efficiency" in data
            else 1.0
        )
        dispatch.loc[mask & dispatch["grid_import_delivered"].isna(), "grid_import_delivered"] = (
            dispatch.loc[mask, "grid_import"] * grid_eta
        )
        dispatch.loc[mask & dispatch["grid_export_delivered"].isna(), "grid_export_delivered"] = (
            dispatch.loc[mask, "grid_export"] * grid_eta
        )
    ...
    for r in resources:
        ...
        res_inv_capex = scalarize(
            data["res_inverter_specific_investment_cost_per_kw_ac"], resource=r
        )
        res_grant = scalarize(data["res_grant_share_of_capex"], resource=r)
        res_life = scalarize(data["res_lifetime_years"], resource=r)
        res_inv_life = (
            scalarize(data["res_inverter_lifetime_years"], resource=r)
            if "res_inverter_lifetime_years" in data
            else res_life
        )
        ...
        res_inv_ann = (
            res_inv_caps[r] * res_inv_capex * (1.0 - res_grant) * _crf(res_wacc, res_inv_life)
        )
        ...
        annuity_rows.append(
            {"Technology (lifetime)": f"{r_label} ({int(res_life)}y)", "Annuity [/yr]": res_ann}
        )
        annuity_rows.append(
            {
                "Technology (lifetime)": f"{r_label} inverter ({int(res_inv_life)}y)",
                "Annuity [/yr]": res_inv_ann,
            }
        )
        upfront_rows.append(
            {
    ...
    bat_inv_life = (
        float(safe_float(data["battery_inverter_lifetime_years"]))
        if "battery_inverter_lifetime_years" in data
        else bat_life
    )
    ...
    annuity_rows.append(
        {"Technology (lifetime)": f"Battery ({int(bat_life)}y)", "Annuity [/yr]": ann_bat}
    )
    annuity_rows.append(
        {
            "Technology (lifetime)": f"Battery inverter ({int(bat_inv_life)}y)",
            "Annuity [/yr]": ann_bat_inv,
        }
    )
    upfront_rows.append(
        {
    ...
    annuity_rows.append(
        {"Technology (lifetime)": f"Generator ({int(gen_life)}y)", "Annuity [/yr]": ann_gen}
    )
    upfront_rows.append(
        {
    ...
    for s in scenarios:
        ...
        bat_kg_s = (
            cap_bat_kwh
            * scalarize(data["battery_embedded_emissions_kgco2e_per_kwh"], scenario=s)
            / max(bat_life, 1e-12)
        )
        gen_kg_s = (
            cap_gen_kw
            * scalarize(data["generator_embedded_emissions_kgco2e_per_kw"], scenario=s)
            / max(gen_life, 1e-12)
        )
    ...
    embodied_rows.append(
        {
            "Technology": "Generator",
            "Lifetime [y]": int(gen_life),
            "Embodied Emissions [kg/yr]": gen_emb_exp,
            "Embodied Cost [/yr]": gen_emb_cost_exp,
        }
    )
    embodied_rows.append(
        {
            "Technology": "Battery",
            "Lifetime [y]": int(bat_life),
            "Embodied Emissions [kg/yr]": bat_emb_exp,
            "Embodied Cost [/yr]": bat_emb_cost_exp,
        }
    )

    bat_fom = cap_bat_kwh * bat_capex * scalarize(data["battery_fixed_om_share_per_year"])
    bat_inv_fom = (
        cap_bat_inv_kw * bat_inv_capex * scalarize(data["battery_inverter_fixed_om_share_per_year"])
    )
    ...
    for s in scenarios:
        scen_dispatch = (
            dispatch[dispatch["scenario"] == s].sort_values("period").reset_index(drop=True)
        )
        ...
        grid_ren_share = (
            scalarize(data["grid_renewable_share"], scenario=s)
            if "grid_renewable_share" in data
            else 0.0
        )
        ...
        if "resource_availability" in data and "res_inverter_efficiency" in data:
            for r in resources:
                avail = np.asarray(
                    data["resource_availability"].sel(scenario=s, resource=r).values, dtype=float
                )
                raw_ac_potential = (
                    avail * res_caps[r] * scalarize(data["res_inverter_efficiency"], resource=r)
                )
                inverter_cap = res_inv_caps[r]
                res_potential += float(np.minimum(raw_ac_potential, inverter_cap).sum())
                res_inverter_clipping += float(
                    np.maximum(raw_ac_potential - inverter_cap, 0.0).sum()
                )
        ...
        if "grid_import_price" in data:
            prices = np.asarray(
                data["grid_import_price"].sel(scenario=s).values, dtype=float
            ).reshape(-1)
        ...
        if "grid_export_price" in data:
            prices = np.asarray(
                data["grid_export_price"].sel(scenario=s).values, dtype=float
            ).reshape(-1)
        ...
        if "res_production_subsidy_per_kwh" in data:
            for r in resources:
                ...
                if col in scen_dispatch.columns:
                    subsidy = scalarize(
                        data["res_production_subsidy_per_kwh"], scenario=s, resource=r
                    )
                    res_subsidy_revenue += float(
                        np.sum(scen_dispatch[col].to_numpy(dtype=float) * subsidy)
                    )
        ...
        grid_em_factor = (
            scalarize(data["grid_emissions_factor_kgco2e_per_kwh"], scenario=s)
            if "grid_emissions_factor_kgco2e_per_kwh" in data
            else 0.0
        )
        ...
        for r in resources:
            scope3 += (
                res_caps[r]
                * scalarize(data["res_embedded_emissions_kgco2e_per_kw"], scenario=s, resource=r)
                / max(scalarize(data["res_lifetime_years"], resource=r), 1e-12)
            )
        scope3 += (
            cap_bat_kwh
            * scalarize(data["battery_embedded_emissions_kgco2e_per_kwh"], scenario=s)
        ...
        annual_variable_cost = (
            fuel_cost + grid_import_cost - grid_export_revenue - res_subsidy_revenue
        )
    ...
    if not kpis.empty:
        ...
        kpis = pd.concat(
            [kpis.drop(columns=["weight"]), pd.DataFrame([expected])], ignore_index=True
        )

    expected_row = (
        select_kpi_row(kpis, data, mode="expected", scenario_label=None)
        if not kpis.empty
    ...
    expected_cost_components = pd.DataFrame(
        [
            {
                "Component": "Annualized renewable CAPEX",
                "Value": float(safe_float(expected_row.get("res_investment_annuity_cost", 0.0))),
                "Unit": "/yr",
            },
            {
                "Component": "Annualized renewable inverter CAPEX",
                "Value": float(
                    safe_float(expected_row.get("res_inverter_investment_annuity_cost", 0.0))
                ),
                "Unit": "/yr",
            },
            {
                "Component": "Annualized battery CAPEX",
                "Value": float(
                    safe_float(expected_row.get("battery_investment_annuity_cost", 0.0))
                ),
                "Unit": "/yr",
            },
            {
                "Component": "Annualized battery inverter CAPEX",
                "Value": float(
                    safe_float(expected_row.get("battery_inverter_investment_annuity_cost", 0.0))
                ),
                "Unit": "/yr",
            },
            {
                "Component": "Annualized generator CAPEX",
                "Value": float(
                    safe_float(expected_row.get("generator_investment_annuity_cost", 0.0))
                ),
                "Unit": "/yr",
            },
            {
                "Component": "Renewable fixed O&M",
                "Value": float(safe_float(expected_row.get("res_fixed_om_cost", 0.0))),
                "Unit": "/yr",
            },
            {
                "Component": "Renewable inverter fixed O&M",
                "Value": float(safe_float(expected_row.get("res_inverter_fixed_om_cost", 0.0))),
                "Unit": "/yr",
            },
            {
                "Component": "Battery fixed O&M",
                "Value": float(safe_float(expected_row.get("battery_fixed_om_cost", 0.0))),
                "Unit": "/yr",
            },
            {
                "Component": "Battery inverter fixed O&M",
                "Value": float(safe_float(expected_row.get("battery_inverter_fixed_om_cost", 0.0))),
                "Unit": "/yr",
            },
            {
                "Component": "Generator fixed O&M",
                "Value": float(safe_float(expected_row.get("generator_fixed_om_cost", 0.0))),
                "Unit": "/yr",
            },
            {
                "Component": "Fuel cost (expected)",
                "Value": float(safe_float(expected_row.get("fuel_cost", 0.0))),
                "Unit": "/yr",
            },
            {
                "Component": "Grid import cost (expected)",
                "Value": float(safe_float(expected_row.get("grid_import_cost", 0.0))),
                "Unit": "/yr",
            },
            {
                "Component": "Grid export revenue (expected)",
                "Value": -float(safe_float(expected_row.get("grid_export_revenue", 0.0))),
                "Unit": "/yr",
            },
            {
                "Component": "RES production subsidy (expected)",
                "Value": -float(safe_float(expected_row.get("res_subsidy_revenue", 0.0))),
                "Unit": "/yr",
            },
            {
                "Component": "Lost load penalty (expected)",
                "Value": float(safe_float(expected_row.get("lost_load_cost", 0.0))),
                "Unit": "/yr",
            },
            {
                "Component": "Emissions cost (scope 1 + 2 + 3, expected)",
                "Value": float(safe_float(expected_row.get("emissions_cost", 0.0))),
                "Unit": "/yr",
            },
            {
                "Component": "TOTAL (Expected)",
                "Value": float(safe_float(expected_row.get("reported_total_annual_cost", 0.0))),
                "Unit": "/yr",
            },
        ]
    )
            ...
            [
                {
                    ...
                    "Annual FOM [/yr]": float(
                        safe_float(expected_row.get("res_fixed_om_cost", 0.0))
                    ),
                },
                {
                    "Technology": "Renewable inverters",
                    "Annual FOM [/yr]": float(
                        safe_float(expected_row.get("res_inverter_fixed_om_cost", 0.0))
                    ),
                },
                {
                    "Technology": "Battery",
                    "Annual FOM [/yr]": float(
                        safe_float(expected_row.get("battery_fixed_om_cost", 0.0))
                    ),
                },
                {
                    "Technology": "Battery inverter",
                    "Annual FOM [/yr]": float(
                        safe_float(expected_row.get("battery_inverter_fixed_om_cost", 0.0))
                    ),
                },
                {
                    "Technology": "Generator",
                    "Annual FOM [/yr]": float(
                        safe_float(expected_row.get("generator_fixed_om_cost", 0.0))
                    ),
                },
            ]
        )
    ...
    scenario_variable_costs = pd.DataFrame(
        [
            {
                "Scenario": s,
                "Fuel cost": float(safe_float(kpis[kpis["scenario"] == s]["fuel_cost"].iloc[0]))
                if len(kpis[kpis["scenario"] == s])
                else 0.0,
                "Grid import cost": float(
                    safe_float(kpis[kpis["scenario"] == s]["grid_import_cost"].iloc[0])
                )
                if len(kpis[kpis["scenario"] == s])
                else 0.0,
                "Grid export revenue": float(
                    safe_float(kpis[kpis["scenario"] == s]["grid_export_revenue"].iloc[0])
                )
                if len(kpis[kpis["scenario"] == s])
                else 0.0,
                "RES subsidy revenue": float(
                    safe_float(kpis[kpis["scenario"] == s]["res_subsidy_revenue"].iloc[0])
                )
                if len(kpis[kpis["scenario"] == s])
                else 0.0,
                "Annual variable cost": float(
                    safe_float(kpis[kpis["scenario"] == s]["annual_variable_cost"].iloc[0])
                )
                if len(kpis[kpis["scenario"] == s])
                else 0.0,
    ...
    scenario_emissions = pd.DataFrame(
        [
            {
                "Scenario": s,
                "Scope 1 emissions": float(
                    safe_float(kpis[kpis["scenario"] == s]["scope1_emissions_kgco2e"].iloc[0])
                )
                if len(kpis[kpis["scenario"] == s])
                else 0.0,
                "Scope 2 emissions": float(
                    safe_float(kpis[kpis["scenario"] == s]["scope2_emissions_kgco2e"].iloc[0])
                )
                if len(kpis[kpis["scenario"] == s])
                else 0.0,
                "Scope 3 emissions": float(
                    safe_float(kpis[kpis["scenario"] == s]["scope3_emissions_kgco2e"].iloc[0])
                )
                if len(kpis[kpis["scenario"] == s])
                else 0.0,
                "Total emissions": float(
                    safe_float(kpis[kpis["scenario"] == s]["emissions_kgco2e"].iloc[0])
                )
                if len(kpis[kpis["scenario"] == s])
                else 0.0,
                "Emissions cost": float(
                    safe_float(kpis[kpis["scenario"] == s]["emissions_cost"].iloc[0])
                )
                if len(kpis[kpis["scenario"] == s])
                else 0.0,
                "Weight": weights.get(s, 0.0),
    ...
    scenario_total_operating_costs = pd.DataFrame(
        [
            {
                "Scenario": s,
                "Fixed O&M": float(safe_float(kpis[kpis["scenario"] == s]["fixed_om_cost"].iloc[0]))
                if len(kpis[kpis["scenario"] == s])
                else 0.0,
                "Fuel cost": float(safe_float(kpis[kpis["scenario"] == s]["fuel_cost"].iloc[0]))
                if len(kpis[kpis["scenario"] == s])
                ...
                "Lost load penalty": float(
                    safe_float(kpis[kpis["scenario"] == s]["lost_load_cost"].iloc[0])
                )
                if len(kpis[kpis["scenario"] == s])
                else 0.0,
                "Emissions cost": float(
                ...
                "Total operating cost": float(
                    safe_float(kpis[kpis["scenario"] == s]["total_operating_cost"].iloc[0])
                )
                if len(kpis[kpis["scenario"] == s])
                else 0.0,
                "Weight": weights.get(s, 0.0),
