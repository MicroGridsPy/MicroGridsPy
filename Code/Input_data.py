...
def URL_creation_d(Data_import):
    for value in Data_import:
        ...
        if "param: user" in value:
            user = '&user=' + value[value.index('=')+1:value.index(';')].replace(' ','')
...
def solarPV_parameters(Data_import):
    for value in Data_import:
        ...
        if "param: beta_power" in value:
            beta_power = float(value[value.index('=')+1:value.index(';')].replace(' ',''))
