...
def shear_exp(param_typical_hourly, Z_1, Z_0, Z_rot): #RIGUARDARE LO SHEAR EXPONENT E CHIEDERSI SE NON è MEGLIO PRENDERLO IN INPUT COME VALORE COSTANTE
...
#%% Extrapolate power curve of the turbine from Power_curves excel file and calculate daily energy production of the turbine     #QUI MANCA IL CALCOLO PER TURBINE AD ASSE ORIZZONTALE
            ...
            for hour in range(len(prob_density[month][day])):
                for velocity in range(len(prob_density[month][day][hour])):
                    if prob_density[month][day][hour][velocity] == []:
                        ...
                        En_WT[month][day][hour].append(power_curve[velocity] * prob_density[month][day][hour][velocity])
