...
def grid_availability(average_n_outages, average_outage_duration, project_lifetime, year_grid_connection):
    ...
    times1 = np.linspace(0.001,math.ceil(lambda_TBO*((-math.log(1-0.9999))**(1/k_TBO))),num = 5*10**3)   #creates a vector of times between 0 and the time at which CDF= 0.9999
    times2 = np.linspace(0.001,math.ceil(lambda_OD*((-math.log(1-0.9999))**(1/k_OD))),num = 2*10**4)
    ...
    plt.hist(samples_TBO,  bins = int(math.ceil((max(samples_TBO) - min(samples_TBO))/0.01)),  color='#0504aa', alpha = 0.7,  rwidth=0.85)  
    plt.grid(axis='y', alpha=0.75)
    plt.xlim([min(samples_TBO), max(samples_TBO)])
    plt.xlabel('Time Between Outages [h]',fontsize=10)
    plt.title('Samples Distribution (TBO)',fontsize=10) 
    ...
    plt.hist(samples_OD,  bins = int(math.ceil((max(samples_OD) - min(samples_OD))/0.01)),  color='#0504aa', alpha = 0.7,  rwidth=0.85)  
    plt.grid(axis='y', alpha=0.75)
    plt.xlim([min(samples_OD), max(samples_OD)])
    plt.xlabel('Outage Duration [h]',fontsize=10)
    plt.title('Samples distribution (OD)',fontsize=10) 
