...
def update_parameters(i: int, res_name: str, time_horizon: int, brownfield: bool, land_availability: float, currency: str) -> None:
    ...
    st.session_state.res_inverter_efficiency[i] = st.number_input(
        ...
        min_value=0.0, 
        max_value=1.0, 
        value=float(st.session_state.res_inverter_efficiency[i]), 
        key=f"inv_eff_{i}")
    ...
    st.session_state.res_specific_om_cost[i] = st.number_input(
        ...
        value=float(st.session_state.res_specific_om_cost[i]), 
        key=f"om_cost_{i}")
    ...
    if brownfield:
        ...
        if land_availability > 0:
            st.session_state.res_existing_area[i] = st.number_input(
                f"Existing Area [m2]", 
                value=float(st.session_state.res_existing_area[i]), 
                key=f"exist_area_{i}")
