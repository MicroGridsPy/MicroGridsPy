...
def add_battery_variables(model: Model, settings: ProjectParameters, sets: xr.Dataset) -> Dict[str, linopy.Variable]:
    ...
    if settings.advanced_settings.milp_formulation:
        ...
        # Introduce a binary variable to determine the sign of battery system energy
    ...
    if any(conn_type == 'Connected with the same Inverter as the Battery to the Microgrid' for conn_type in settings.renewables_params.res_connection_types):
        battery_variables['battery_system_feed_in_losses'] = model.add_variables(lower=0, coords=[sets.scenarios, sets.years, sets.periods], name="Feed In Losses - Battery")
        battery_variables['battery_system_charge_losses'] = model.add_variables(lower=0, coords=[sets.scenarios, sets.years, sets.periods], name="Charge Losses - Battery")
        ...
        battery_variables['battery_system_transformation_losses'] = model.add_variables(lower=0, coords=[sets.scenarios, sets.years, sets.periods], name="Transformation Losses - Battery")
