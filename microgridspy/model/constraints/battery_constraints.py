...
def add_battery_constraints(model: Model, settings: ProjectParameters, sets: xr.Dataset, param: xr.Dataset, var: Dict[str, linopy.Variable]) -> None:
    ...
    if settings.project_settings.battery_independence > 0:
        add_min_battery_capacity_constraints(model, settings, sets, param, var)
...
def add_battery_flow_constraints(model: Model, settings: ProjectParameters, sets: xr.Dataset, param: xr.Dataset, var: Dict[str, linopy.Variable]) -> None:
    ...
    model.add_constraints(
        var['battery_max_charge_power'] == (var['battery_units'] * param['BATTERY_NOMINAL_CAPACITY']) / param['MAXIMUM_BATTERY_CHARGE_TIME'],
        name=f"Battery Maximum Charge Power Constraint")
        
    model.add_constraints(
        var['battery_max_discharge_power'] == (var['battery_units'] * param['BATTERY_NOMINAL_CAPACITY']) / param['MAXIMUM_BATTERY_DISCHARGE_TIME'],
        name=f"Battery Maximum Discharge Power Constraint")
    ...
    model.add_constraints(var['battery_outflow'] <= param['DEMAND'],
            name="Battery Maximum Outflow Constraint")
...
def add_min_battery_capacity_constraints(model: Model, settings: ProjectParameters, sets: xr.Dataset, param: xr.Dataset, var: Dict[str, linopy.Variable]) -> None:
    ...
    model.add_constraints(var['battery_units'] * param['BATTERY_NOMINAL_CAPACITY'] >= param['BATTERY_MIN_CAPACITY'],
        name="Battery Minimum Capacity Constraint")
...
def add_battery_single_flow_constraints(model: Model, settings: ProjectParameters, sets: xr.Dataset, param: xr.Dataset, var: Dict[str, linopy.Variable]) -> None:
    ...
    model.add_constraints(
        var['battery_outflow'] <= param['DEMAND'],
        name="Battery Maximum Outflow Constraint")
