...
Many communities in remote regions lack access to modern and sustainable energy services, including reliable electricity. Mini-grids - small local electricity systems built around renewable generation, storage, and backup generators - are one of the main ways to supply them, but designing one means deciding what to build, how large, in what order, and at what cost.
...
General-purpose energy-system frameworks such as Calliope [@calliope], PyPSA [@pypsa], and OSeMOSYS [@osemosys] are powerful open-source tools for energy-system optimization across a range of scales and applications. Applying them to detailed mini-grid planning may nevertheless require additional model development to represent features such as battery ageing, generator part-load operation, staged investment under demand growth, lost-load treatment, and scenario-dependent uncertainty.
